...
## Text Indexes vs Bloom Filter {#text-index-vs-bloom-filter}

Although bloom filter indexes can sometimes be used to speed up string predicates, they are fundamentally different from text indexes in both design and intended use cases:

**Bloom filter indexes**

- Are probabilistic data structures.
- Answer only set-membership questions (i.e. “may contain” vs “definitely does not contain”).
- May produce false positives.
- Operate at the granule level to skip data parts during query execution.
- Do not build a true inverted index of terms.

**Text indexes**

- Build a real inverted index over tokens.
- Store token-level information that enables efficient term lookup.
- Are deterministic (no false positives introduced by the index itself).
- Are specifically optimized for text search workloads.

However, bloom filter indexes are **not designed to implement full-text search semantics**. In particular:

- They do not support advanced tokenization strategies in a structured way.
- They are not optimized for multi-term search.
...
- Efficient evaluation of `hasToken`, `match`, and similar text-search functions.
- Proper tokenization and term lookup.
- Significantly better scalability for large text corpora.

:::note
For full-text search workloads, you should use **text indexes**.
:::
