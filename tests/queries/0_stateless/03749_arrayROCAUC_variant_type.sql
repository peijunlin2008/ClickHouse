...
-- Original failing query from the issue (exact reproduction)
WITH partial_aucs AS (SELECT arrayROCAUC(scores, labels, true, roc_offsets) AS partial_roc_auc FROM (SELECT [0, 0, 2] AS pr_offsets, [0.8, 0.4] AS scores, [1, 0] AS labels, [0, 0, 2, 2] AS roc_offsets UNION ALL SELECT DISTINCT [1, 1, 2, 2] AS roc_offsets, [0.35, 0.1] AS scores, [1, 1, 2] AS pr_offsets, * QUALIFY isNotDistinctFrom(isNotDistinctFrom(*, isNullable(*)), 0))) SELECT equals(0, isNotDistinctFrom(toNullable(0), isNull(isNotNull(1)))), floor(stddevSamp(partial_roc_auc), 10) FROM partial_aucs WITH TOTALS;
