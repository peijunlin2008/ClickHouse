...
def test_metadata_file_path_security(started_cluster_iceberg_with_spark, storage_type):
    """
    ...
    create_iceberg_table(
        ...
        TABLE_NAME,
        started_cluster_iceberg_with_spark,
        explicit_metadata_path="metadata/v3.metadata.json"
    )
    ...
    create_iceberg_table(
        ...
        TABLE_NAME,
        started_cluster_iceberg_with_spark,
        explicit_metadata_path="metadata/v5.metadata.json"
    )

    # Test 12: Another valid path with nested directory
    instance.query(f"DROP TABLE IF EXISTS {TABLE_NAME}")
    create_iceberg_table(
        storage_type,
        instance,
        TABLE_NAME,
        started_cluster_iceberg_with_spark,
        explicit_metadata_path=f"/var/lib/clickhouse/user_files/iceberg_data/default/{TABLE_NAME}/metadata/v5.metadata.json"
    )
